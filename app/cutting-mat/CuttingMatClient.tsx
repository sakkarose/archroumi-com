...
function roundSvgNumber(value: number, precision = 3) {
  return Number(value.toFixed(precision));
}

function getRadiusGuidePath(
  originX: number,
  originY: number,
  radius: number,
  segments = 18,
  ...
  const points = Array.from({ length: segments + 1 }, (_, index) => {
    ...
    return {
      x: roundSvgNumber(originX + Math.cos(angle) * radius),
      y: roundSvgNumber(originY - Math.sin(angle) * radius),
    };
  });
...
function CuttingMatPreview({
  ...
  const tickLong = 0.84;
  const rulerLabelOffset = tickLong + 0.55;
  const sideRulerOffset = tickLong + 0.22;
  const rulerMargin = 1.7;
  const matPad = Math.max(
    rulerLabelOffset + rulerMargin,
    sideRulerOffset + rulerMargin,
  );
  const matPadX = matPad;
  const matPadTop = matPad;
  const matPadBottom = matPad;
  ...
  const topLabelY = gridY - rulerLabelOffset;
  const bottomLabelY = gridBottom + rulerLabelOffset;
  const leftLabelX = gridX - sideRulerOffset;
  const rightLabelX = gridRight + sideRulerOffset;
  const guideInk = "#180d08";
  const textInk = "#160d09";
  const labelPlateFill = surfaceColor;
  const angleDash = "0.16 0.18";

  const getGuideLabelWidth = (value: string) => 0.3 + value.length * 0.28;
        ...
        >
          ...
          {showRadiusGuides
            ? radii.map((radius) => (
                <path
                  key={radius}
                  d={getRadiusGuidePath(gridX, gridBottom, radius)}
                  fill="none"
                  stroke={guideColor}
                  strokeWidth="0.065"
                  opacity="0.78"
          ...
          {angleLines.map((angle) => {
            ...
            const firstRadius = radii[0] ?? 10;
            const labelDistance = firstRadius + 1.2;
            const label = `${angle} deg`;
            const labelWidth = getGuideLabelWidth(label);
            const labelX = gridX + Math.cos(radians) * labelDistance;
            const labelY = gridBottom - Math.sin(radians) * labelDistance;
          ...
          {showEdgeTicks
            ? Array.from({ length: width + 1 }, (_, index) => {
                ...
                const x = gridX + index;
                const topTickStart = gridY - tickLong;
                const topTickEnd = longTick ? gridY : topTickStart + tickShort;
                const bottomTickEnd = gridBottom + tickLong;
                const bottomTickStart = longTick
                  ? gridBottom
                  : bottomTickEnd - tickShort;
                return (
                  <g key={`tick-x-${index}`}>
                    <line
                      x1={x}
          ...
          {showEdgeTicks
            ? Array.from({ length: height + 1 }, (_, index) => {
                const longTick = index % 5 === 0;
                const y = gridY + index;
                const leftTickStart = gridX - tickLong;
                const leftTickEnd = longTick ? gridX : leftTickStart + tickShort;
                const rightTickEnd = gridRight + tickLong;
                const rightTickStart = longTick
                  ? gridRight
                  : rightTickEnd - tickShort;
                return (
                  <g key={`tick-y-${index}`}>
                    <line
                      x1={leftTickStart}
          ...
          {showTickMarks
            ? radii.map((radius) => {
                const tickAngles = Array.from(
                  { length: Math.ceil(90 / angularSpacing) },
                  (_, index) => angularSpacing * index + angularSpacing / 2,
                ).filter((angle) => angle > 0 && angle < 90);

                return tickAngles.map((angle) => {
                  const radians = (angle * Math.PI) / 180;
                  const x1 = gridX + Math.cos(radians) * (radius - 0.42);
                  const y1 = gridBottom - Math.sin(radians) * (radius - 0.42);
                  const x2 = gridX + Math.cos(radians) * (radius + 0.42);
                  const y2 = gridBottom - Math.sin(radians) * (radius + 0.42);
          ...
          {showRadiusGuides
            ? radii.map((radius, index) => {
                ...
                const labelWidth = getGuideLabelWidth(label);
                const x = gridX + radius;
                const y = gridBottom - 1;

                return (
                  <g key={`radius-label-${radius}`}>
                    <rect
                      x={x - labelWidth / 2}
                      y={y - 0.42}
                      width={labelWidth}
    ...
    <main className="min-h-screen bg-[radial-gradient(circle_at_top,_rgba(255,111,46,0.16),_transparent_24%),linear-gradient(180deg,#151515_0%,#090909_100%)] px-3 py-3 text-white md:px-4 md:py-4 lg:px-5">
      <div className="mx-auto grid max-w-[1820px] gap-4 xl:grid-cols-[minmax(0,1.55fr)_420px] 2xl:grid-cols-[minmax(0,1.72fr)_460px]">
        ...
        <aside className="xl:sticky xl:top-4 xl:h-[calc(100vh-2rem)] xl:overflow-y-auto xl:pr-1">
          ...
          >
            <div className="space-y-3.5">
              ...
              <div className="subpanel space-y-2">
                {radii.map((radius, index) => (
                  <label key={index} className="grid gap-2 sm:grid-cols-[72px_1fr] sm:items-center">
                      ...
                      onChange={(value) =>
                        setRadii((current) =>
                          current.map((item, itemIndex) =>
                            itemIndex === index ? clamp(value, 2, 60) : item,
                          ),
                        )
                      }
                    />
              ...
              <div className="subpanel">
                <div className="flex items-center gap-4">
                  ...
                  />
                  <span className="text-sm text-white/55">{angularSpacing}°</span>
                </div>
              </div>
            </div>
          </ControlCard>
          ...
          <ControlCard
            title="Export"
            description="Download the current design as SVG, PNG, or PDF."
          >
            <div className="grid gap-3 sm:grid-cols-[minmax(0,1fr)_auto]">
              ...
              <button
                ...
                className="inline-flex min-h-11 items-center justify-center rounded-[18px] bg-[#ff6f2e] px-5 text-sm font-medium text-black transition hover:bg-[#ff8b58] disabled:cursor-wait disabled:opacity-70"
              >
                {isExporting ? "Exporting..." : `Download ${format}`}
              </button>
            </div>
          </ControlCard>
          </div>
